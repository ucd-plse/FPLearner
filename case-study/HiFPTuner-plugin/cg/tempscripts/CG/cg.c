...
int main(int argc, char *argv[])
{
  ...
  for (it = 1; it <= 1; it++) {
    ...
    norm_temp2 = 1.0 / sqrtf(norm_temp2);
  ...
  printf(" Benchmark completed\n");
   ...
   zeta_verify_value = 17.130235054029;
    ...
    // if (zeta_flag) {
    //     zeta_verify_value = zeta;
    //     // record zeta_verify_value
    //     FILE *fp_zeta = fopen("./zeta_verify_value.txt", "w");
    //     fprintf(fp_zeta, "%20.13E\n", zeta_verify_value);
    // } else {
    //     FILE *fileStream;
    //     char line [9999];
    //     fileStream = fopen ("./zeta_verify_value.txt", "r");
    //     fgets (line, sizeof(line), fileStream);
    //     fclose(fileStream);

    //     zeta_verify_value = atof(line);
    //     printf("zeta_verify_value is %20.13E\n", zeta_verify_value);
    // }
                      ...
                      double *rnorm)
{
  ...
  for (cgit = 1; cgit <= cgitmax; cgit++) {
    ...
    */
    ...
    rho = 0.0;
    for (j = 0; j < lastcol - firstcol + 1; j++) {
      zz[j] = zz[j] + alpha*pp[j];
      rr[j] = rr[j] - alpha*qq[j];
    }
                   ...
                   double shift)
{
  ...
  float rcondrcond = rcond;

  float shiftshift = shift;
  ...
  //---------------------------------------------------------------------
  size = 1.0;
  ratio = pow(rcondrcond, (1.0 / (double)(n)));
