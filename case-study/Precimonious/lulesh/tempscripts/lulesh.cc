...
{
   float bisectX0 = double(0.5) * (x3 + x2 - x1 - x0);
   float bisectY0 = double(0.5) * (y3 + y2 - y1 - y0);
   float bisectZ0 = double(0.5) * (z3 + z2 - z1 - z0);
...
#pragma omp parallel for firstprivate(numElem)
   for (int i = 0 ; i < numElem ; ++i ) {
      ...
      float x6 = domain.x(n6) ;
      float x7 = domain.x(n7) ;

      float y0 = domain.y(n0) ;
      float y1 = domain.y(n1) ;
